...
def parse_documents_from_text(
    *,
    ...
    # Thêm lệnh print để theo dõi Terminal
    print(f"\n⚙️ Đang xử lý file: {filename}")
    print(f"   Yêu cầu từ giao diện: {default_doc_type}")
    
    # Chỉ cần có chữ "Tự động" là kích hoạt mắt thần (tránh lỗi sai dấu cách)
    if "Tự động" in default_doc_type:
        doc_type = auto_detect_doc_type(filename, cleaned)
        print(f"   => Mắt thần quyết định: {doc_type}")
    else:
        doc_type = normalize_doc_type(default_doc_type)
        print(f"   => Gán cứng thành: {doc_type}")
